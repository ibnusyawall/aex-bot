...
class LibBot {
	...
	menuBot_(callback) {
		const menuBot = () => {
			...
			💌  *'!spam <number> <jumlah>'* Spam WA by AEX-BOT : key ? PM!
	...
	spamWaBot(key, callback) {
		...
		/* Kerangka cara kerja:
			Spam dapat digunakan menggunakan api key yang telah disediakan dev
			Spam menggunakan limit per hit
			Menggunakan perulangan for untuk menambah kinerja mengirim pesan
			Key berupa barcode random yg akan diberikan setiap group
			User hanya bisa menggunakan fitur ini via barcode berdasarkan instruksi yg diberikan dev
			Key menggunakan bs64 enc , split, reverse, join function demi keamanan
			Gass Boss!
		*/ 
		run().catch(error => console.error(error.stack));
